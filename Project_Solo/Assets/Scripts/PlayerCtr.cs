...
////Attack / Ray Cast
    ...
    {
        if (_moveVector != Vector3.zero && !isAttack)
        {
        ...
        if(isAttack)
        {
            transform.rotation = Quaternion.Lerp(transform.rotation, attackRotation * Quaternion.Euler(Vector3.up * 60), _rotationSpeed * Time.deltaTime);
